...
export const toDateTimeStringY = (d: string) => {
    const date = new Date(Date.parse(d));
    return `${("0000" + date.getFullYear()).slice(-4)}-${("00" + (date.getMonth() + 1)).slice(-2)}-${("00" + date.getDate()).slice(-2)}T${("00" + date.getHours()).slice(-2)}:${("00" + date.getMinutes()).slice(-2)}:${("00" + date.getSeconds()).slice(-2)}`;
};
